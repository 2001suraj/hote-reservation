...
class IamgeStorage{
  FirebaseFirestore storage = FirebaseFirestore.instance;
    ...
    try {
      ...
      var ref = FirebaseStorage.instance.ref().child('hotel').child(name);
      ref.putFile(photo);
      uploadTask = ref.putFile(photo);
      final snap = await uploadTask.whenComplete(() {});
      final urls = await snap.ref.getDownloadURL();
      var user = await storage.collection('hotel').doc(name);
 ...
 Future<void> addPhotos({
    ...
    try {
      ...
      for (int i = 0; i < photos.length; i++) {
        ...
        UploadTask? uploadTask;
        var ref = FirebaseStorage.instance.ref().child('hotel').child(name + '_$i');
      ...
      var user = storage.collection('hotel').doc(name);
